...
- Terminai turi buti produkto terminai, ne UI paaiskinimai: `Work Act`, `Template`, `Output Layout`, `Generated document`, `Uploaded document`, `Signed document`.
...
| Templates CRUD | `+ New template` forma, detail/edit panel, `Duplicate`, `Archive/Restore`; inline work row add/remove/text-edit; `isActive` flag — ne destruktyvus archyvavimas |
| Output Layouts editor | Advanced/admin sekcijų redaktorius su merge fields, reset/save logika, localStorage; sekcijų turinys perduodamas į document-service payload |
...
| ~~B-17~~ | ~~**Template Generation module split**~~ | **Done:** pridetas `Templates` sidebar modulis; `Documents` nebemaiso template generation panelio su repository/search/detail/upload sritimi. Dabartinis `/templates` landing yra Template list/management, o `/templates/:templateId` atidaro editor-first konfiguravima. |
| ~~B-18~~ | ~~**Template Generation sub-tabs**~~ | **Done:** ankstesni top-level sub-tabs isimti is `Templates` landing. Work/Defect/Commercial source flows lieka techniniame kode/linkuose, o sidebar `Templates` atidaro Template list; konkretus sablonas redaguojamas detail route. |
...
| ~~B-23~~ | ~~**Templates CRUD**~~ | **Done/current:** `Templates` workspace turi `/templates` sarasa su search/filter/table veiksmais ir `/templates/:templateId` detail editoriu su Company, Entry person, Template name, Service type, Equipment/Hospitals/Work Equipment linkais, Umo editoriumi, merge fields, `Save`, `Duplicate`, `Archive`, `Cancel`. `Open in advanced editor` ir Templates inline work row redagavimas buvo pasalinti is aktyvios architekturos; Work rows priklauso `Work Acts`. |
...
- Template = aparatu/proceduru checklist'as, kopijuojamas i konkretu Work Act. Tomis radiniuose tas pats konceptas vadinamas Work List Template.
- Output Layout = standartizuota spausdinama forma Carbone/LibreOffice generavimui. Tai advanced/admin sritis, ne kasdienis template pasirinkimas.
- Work Act flow igyvendintas: Draft record -> auto-prefill equipment is case/equipment arba manual equipment search/add/remove -> apply Template -> edit isolated work rows/work description -> generate Work Act document draft.
...
| # | Užduotis | Aprašas |
|---|---|---|
| ~~B-24~~ | ~~**fodt template upload/export**~~ | **Done:** Output Layouts tab'e pridėtas "Export sections as .fodt" ir "Upload .fodt template" veiksmai, `document-service` turi `POST /template/upload` ir `GET /template/download/:fileName`, o sekcijų editoriaus turinys gali tapti realiu `.fodt` šablonu Carbone generavimui. |
...
| ~~B-27~~ | ~~**Template applicability rules**~~ | **Done:** Templates registry turi Search/Find/Clear, statuso ir entry person filtrus, linked service types/equipment/hospitals/work equipment laukus, entry person/date metadata, o Work Act template pickeris rodo applicability atitinkancius sablonus. |
...
| ~~B-32~~ | ~~**User-accessible visual template editor**~~ | **Done as Umo MVP:** `Templates` detail route turi Umo editoriu, HTML/text/json content snapshots, merge-field insertion, visual HTML persistence ir save/load per `document-service`; strukturuotos work rows ir applicability metadata lieka default generavimo source. |
...
| B-39 | **Document repository workflow polish** | Started: Documents turi row-level `Delete` su confirmation, frontend state cleanup ir `document-service` generated/file custody cleanup pagal document id. Taip pat sutvarkyta Templates/Documents riba: Templates nebegeneruoja persisted Documents tiesiogiai, template API eina per `/api/templates/`, o Documents ignoruoja senus `sourceType=template` artefaktus. Toliau: geresni empty states, file history row expansion, be archyvavimo kol nera retention dizaino. |
...
| B-41 | **Umo template editor V2** | Started: Templates UX atskirtas i `/templates` list/management ir `/templates/:templateId` editor-first detail su Umo primary editoriumi bei metadata/applicability/merge-fields supporting paneliais. Toliau po papildomo Tomis crawl: table editing, stronger merge fields, logo/image placeholders, autosave, dirty-state warning, revert, duplicate as personal template, version history. |
...
| B-45 | **Tomis deep crawl completion** | Naudoti `docs/TOMIS_CRAWL_PLAYBOOK.md`: Work Acts, Defect Acts, Commercial Offers, Templates, Output Layouts, preview/export/email/upload, admin/permissions, status and close behavior. |
