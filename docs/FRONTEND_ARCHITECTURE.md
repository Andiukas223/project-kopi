...
This document summarizes the current frontend architecture after the Vue 3 foundation, shell, shared component, routing/state, Documents migration, Templates migration, Work Acts migration, and cleanup passes.
...
- The Templates module surface: Work List Template picker/configurator, metadata fields, applicability comboboxes, action buttons, merge fields, and same-page Umo editor.
...
- `src/js/render.js` still renders Service, Contracts, Customers, Equipment, Calendar, Admin, dormant prototype pages, generated document print preview, and feedback overlays.
...
The old legacy Templates landing renderer has been removed. The active Templates route is rendered by `src/modules/templates/TemplatesPage.vue`; legacy support is limited to delegated Work List Template save/delete/rich editor/combobox handlers. Template applicability is now owned by the Vue Work Acts view model.
...
- nginx proxies document APIs through `/api/documents/` to `document-service:3001`.
...
- `src/modules/templates/TemplatesPage.vue` owns template metadata, merge fields, applicability, save/load actions, and generation actions.
