import {
  ...
  invoices, jobs, partsRequests, pipelineStages, quotations, roles, templates, users, vendorReturns, workActs
} from "./data.js";
...
const moduleSpecs = {
  ...
  templates: {
    ...
    roles: "Service and Service Manager maintain procedure templates. Admin can manage visual template/layout behavior. Manager can review as read-only.",
    submodules: "Work List Template configurator - applicability links - same-page visual editor",
    pipeline: "Select template -> set company/person/name/service type -> link equipment/hospitals/work equipment -> edit visual body -> save reusable template",
    actions: "Save - Delete - Cancel - rich text formatting",
    planned: "Visual editor V2 with table editing, merge fields, logo/image placeholders, autosave, dirty-state warning, revert, and version history"
  },
  templategen: {
    purpose: "Reusable Work List Template configuration separated from the Documents repository.",
    roles: "Service and Service Manager maintain procedure templates. Admin can manage visual template/layout behavior. Manager can review as read-only.",
...
function documentPrintPreviewOverlay() {
  ...
  return `
    <div class="print-preview-overlay" role="dialog" aria-modal="true" aria-label="Generated document preview">
      <div class="print-preview-window" tabindex="-1">
        ...
        <div class="print-preview-ribbon">
          <button class="preview-tool primary-preview-tool" type="button" data-doc-delivery-action="print"><strong>Print</strong><span>Browser dialog</span></button>
          <button class="preview-tool" type="button" data-doc-delivery-action="quick-print"><strong>Quick Print</strong><span>Record action</span></button>
          ...
          <div class="preview-menu-wrap">
            <button class="preview-tool" type="button" data-doc-preview-export><strong>Export</strong><span>PDF options</span></button>
            ${state.printPreviewExportOpen ? printPreviewExportMenu(generatedFile) : ""}
          </div>
          <button class="preview-tool" type="button" data-doc-preview-email><strong>Email</strong><span>PDF compose</span></button>
          <button class="preview-tool danger-tool" type="button" data-doc-preview-close><strong>Close</strong><span>Return to table</span></button>
        </div>
...
// Router
// ---------------------------------------------------------------------------
export function renderDocumentRouteOverlays() {
  return `${documentPrintPreviewOverlay()}${feedbackSelectionOverlay()}${feedbackOverlay()}${feedbackLauncher()}`;
}
...
export function renderPage() {
  ...
  switch (state.page) {
    ...
    case "documents": pageHtml = ""; break;
    case "workacts": pageHtml = ""; break;
    case "templates":
    case "templategen": pageHtml = ""; break;
